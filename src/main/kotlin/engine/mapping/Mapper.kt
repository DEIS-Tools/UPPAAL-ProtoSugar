...
abstract class Phase {
    ...
    protected inline fun <reified T : UppaalPojo> register(noinline handler: (List<PathNode>, T) -> List<MapperError>, prefix: List<Class<out UppaalPojo>> = ArrayList()) {
        handlers.add(Triple(typeOf<T>(), prefix.plus(T::class.java), handler))
    }

    inline fun <reified T : UppaalPojo> visit(path: List<PathNode>, element: T): List<MapperError> {
        for (handler in handlers.filter { it.first == typeOf<T>() })
            if (pathMatchesFilter(handler.second, path))
                @Suppress("UNCHECKED_CAST")
                return (handler.third as (List<PathNode>, T) -> List<MapperError>)(path, element)
        return listOf()
    }
...
class MapperError(pathList: List<PathNode>,
...
/** This is defined alongside the "MapperError" class to help generate the start/end line/column values needed for errors **/
